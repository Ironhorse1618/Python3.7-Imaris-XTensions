    ...
    #<CustomTools>
        #<Menu>
            #<Submenu name="Filaments Functions">
                #<Item name="Filament Sholl Analysis1 beta" icon="Python3">
                    #<Command>Python3XT::XT_MJG_Filament_ShollAnalysis1_beta(%i)</Command>
                #</Item>
            #</Submenu>
       #</Menu>
       #<SurpassTab>
...
#aImarisId=0
def XT_MJG_Filament_ShollAnalysis1_beta(aImarisId):
    ...
    #Test is the segmentlength is too short to reoreeder or fil spots. Currently set to 5
            if len(vSegmentPositionsWorking)<5:
            ...
            if qSpotinQuestion==True:#Test if there are any questionable intersections
                ...
                for qIndex in range (len(qUniqueIndex)):
                    ...
                    if qUniqueCounts[qIndex]==2:
                        vSpotPositionCurrentShollSphere.append(qUniqueNodes[qIndex])
    ...
    # #Make spot position conect to filament as spine attachment point
        ...
        #Grab New Filament Spine Statistics for attachment point distance.
                ...
                for i in range (len(vSpotPositionAllShollSpheresPerFilament)):
                    # wCompleteShollSpotDistAlongFilamentStat.append(vNewFilamentSpineAttPtDist[vNewFilamentStatIdsDist.index(vNewFilamentStatIdsPosX[vNewFilamentSpinePtPosX.index(wNewSpotsOnFilament[i][0])])])
                    wCompleteShollSpotDistAlongFilamentStatWorking.append(vNewFilamentSpineAttPtDist[vNewFilamentStatIdsDist.index(vNewFilamentStatIdsPosX[vNewFilamentSpinePtPosX.index(vSpotPositionAllShollSpheresPerFilament[i][0])])])
    ###########################
    #Create stats for distance along dendrite to starting point
            ...
            vNewShollSpotsPerFilament.AddStatistics(vSpotsStatNames, wCompleteShollSpotDistAlongFilamentStatWorking,
                                          ...
                                          vSpotsStatFactorName, vSpotsvIds)
    ################################
            # [vNewStatSpotShollDistance[i]/wCompleteShollSpotDistAlongFilamentStatWorking[i] for i in range(len(vNewStatSpotShollDistance))]
            vSpotsStatUnits=['']*len(vSpotPositionAllShollSpheresPerFilament)
