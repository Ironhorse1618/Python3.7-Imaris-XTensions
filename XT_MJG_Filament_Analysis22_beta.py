    ...
    #<CustomTools>
        #<Menu>
            #<Submenu name="Filaments Functions">
                #<Item name="Filament Analysis22 beta" icon="Python3">
                    #<Command>Python3XT::XT_MJG_Filament_Analysis22_beta(%i)</Command>
                #</Item>
            #</Submenu>
       #</Menu>
       #<SurpassTab>
...
import ImarisLib
#aImarisId=0
def XT_MJG_Filament_Analysis22_beta(aImarisId):
    ...
    def Filament_options():
        ...
        global vOptionFilamentSpotThreshold, vOptionDendriteToDendriteContact,NamesSpots, NamesSurfaces
        if (var1.get() == 0) and (var2.get() == 0) and (var4.get() == 0) and (var5.get() ==0) and (var7.get() ==0):
    ...
    # repeating elements.
        ...
        ###############################################################################
            ...
            xNumberofContactsPerDendriteWorking=[]
        ...
        #For bouton detect and Spots display
                if vOptionFilamentToSpotsFill==1 or vOptionFilamentBoutonDetection==1 or vOptionFilamentCloseToSpots==1 or vOptionIntensity==1 or vOptionDendriteToDendriteContact==1:
        ###########################
        ###########################
        #ReOrder segment
                #Reordering
    ...
    ##############
            ...
            #Find and search for indices of All points that match current node/branch point or startpoint
                for b in range (len(wFilamentBranchPointIndex)):
                    ...
                    xBranchSharingNodes=DendriteCurrentWorkingPositions=list(itemgetter(*xNodeIndex)(vAllSegmentIdsPerFilamentDendriteIndex))
                ...
                # xBranchSharingNodesALL=np.asarray(xBranchSharingNodesALL)



                #xBranchSharingNodesALL=list(set(chain(*xBranchSharingNodesALL)))
    ...
    #############
                ...
                #Measure Current segment distance to all Filament point except the current one not to spines.
                    if xDendriteTypeCurrent==0:
                        ...
                        # #per current dendrite segment - min distance of each Denrite point to working Filament - raddi of current point
                        # xShortestDistanceDendriteToDendrite=xDendriteToDendriteDistanceArray.min(axis=1)-xDendriteCurrentWorkingRadii
                        # #xShortestDistanceDendriteToDendrite=where(wDendriteToDendriteDistanceArray>0, wDendriteToDendriteDistanceArray,np.inf).min(axis=1)
                        # xShortestDistanceDendriteToDendrite[xShortestDistanceDendriteToDendrite<0]=0
                        ...
                        # xtest=xBranchSharingNodesALL
                        # xtest.append([1,4,5])
                        # xNodeAttachedSegments=unique(xtest[[i for i, lst in enumerate(xtest) if vBranchIndex in lst]]).tolist()

                        # [i for i, lst in enumerate(xtest) if vBranchIndex in lst]#creates [0,1] identifies index of row where vbranchindex is present
                        # ([xtest[i] for i in [i for i, lst in enumerate(xtest) if vBranchIndex in lst]])
                        ...
                        # ##list unique branch indicies that connected via nodes
                        # xNodeAttachedSegments=unique(xBranchSharingNodesALL[[i for i, lst in enumerate(xBranchSharingNodesALL) if vBranchIndex in lst]]).tolist()
                        # xNodeAttachedSegments=[xNodeAttachedSegments]
                        # xNodeAttachedSegments = list(itertools.chain(*xNodeAttachedSegments))

                        # xNodeAttachedSegments=[0,1,3,2#this is thefinal result]
                        ...
                        ##############################################################################
                        ##############################################################################
                        def CalculateContactSpotSize(Id,Positions,w):
                            global xContinuousContactSpotSize, xContactSpotPostionWorking

                            if len(Positions)==1:
                                xContinuousContactSpotSize=xDendriteCurrentWorkingRadii[xCBIarray[0,w]]
                            else:
                                x = np.array([ Positions[index][0] for index in Id ])
                                y = np.array([ Positions[index][1] for index in Id ])
                                z = np.array([ Positions[index][2] for index in Id ])
                                dist_array = (x[:-1]-x[1:])**2 + (y[:-1]-y[1:])**2 +(z[:-1]-z[1:])**2
                                xContinuousContactSpotSize=np.sum(np.sqrt(dist_array))
                            #Find Center position
                            res_list = [xDendriteCurrentWorkingPositions[i] for i in Id]
                            if len([xDendriteCurrentWorkingPositions[i] for i in Id])%2==0:
                                first_middle = int(len(res_list) / 2) - 1
                                second_middle = int(len(res_list) / 2)
                                #xContactSpotPostionWorking=[statistics.mean(k) for k in zip(res_list[first_middle], res_list[second_middle])]
                        ...
                        if xSizeCBIarray==1:#only one contact point
                            ...
                            for w in range (xSizeCBIarray):
                                if (w==xSizeCBIarray-1) and xIsContinuous==False:#Test if last contact point
                                    ...
                                    xCompleteSpotSizeforContact.append(xContinuousContactSpotSize/2)
                        ...
                        ###############################
                                    if xIsContinuous:# but not yet identified as end on continuous contact
                                        ...
                                        if ((xContactTest[2] == 0) and (xContactTest[0] == 0)):
                                            ...
                                            xContactLength=0
                                            continue
                                        elif ((xContactTest[0] == 1) and (xContactTest[2] == 1)) and ((xCBIarray[1,w]==(xCBIarray[1,w+1])-1) or (xCBIarray[1,w]==(xCBIarray[1,w+1])-2) or (xCBIarray[1,w]==(xCBIarray[1,w+1])+1)):
                                            ...
                                            if ((xCBIarray[0,w]==(xCBIarray[0,w+1])-1) or (xCBIarray[0,w]==(xCBIarray[0,w+1])-2)) and (xCBIarray[1,w]==(xCBIarray[1,w+1])+1) or (xCBIarray[1,w]+1==(xCBIarray[1,w+1])):
    ...
    #############################################################################
                ...
                if  vStatisticFilamentDendriteDendriteContacts == 0:
                    vNewSpotsDendriteDendriteContacts = vImarisApplication.GetFactory().CreateSpots()
                    vNewSpotsDendriteDendriteContacts.SetName('NO Dendrite-Dendrite Contact -- FilamentId:' + str(vFilamentIds[vFilamentCountActual-1]))
                    vNewSpotsAnalysisFolder.AddChild(vNewSpotsDendriteDendriteContacts, -1)
                else:
                    vNewSpotsDendriteDendriteContacts = vImarisApplication.GetFactory().CreateSpots()
                    vNewSpotsDendriteDendriteContacts.SetName('Intra Dendrite-Dendrite Contacts -' + ' Spots -- FilamentId:' + str(vFilamentIds[vFilamentCountActual-1]))
        ...
        #############################################################################
                    ...
                    vSpotsStatNames=[' Dendrite-Dendrite contact length']*len(xCompleteDendriteDendriteContactSpotPositions)
        ...
        # #Make spot position conect to filament as spine attachment point
            ...
            #################################################################################
                ...
                if len(wSpotsAllIndexPerFilament) != []:
                    vNewSpotsvNewSpotsAlongFilament = vImarisApplication.GetFactory().CreateSpots()
                    vNewSpotsvNewSpotsAlongFilament.SetName('Colocalized-'+ str(vSpots.GetName())+' Spots -- FilamentId:' + str(vFilamentIds[vFilamentCountActual-1]))
