    ...
    #<CustomTools>
        #<Menu>
            #<Submenu name="Filaments Functions">
                #<Item name="Filament Analysis32" icon="Python3">
                    #<Command>Python3XT::XT_MJG_Filament_Analysis32_Imaris10(%i)</Command>
                #</Item>
            #</Submenu>
       #</Menu>
       #<SurpassTab>
...
import ImarisLib
aImarisId=0
def XT_MJG_Filament_Analysis32_Imaris10(aImarisId):
    ...
    def Filament_options():
        global vOptionProcessOnlyDendrites, vOptionFilamentToSpots,vOptionFilamentToSpotsMerge,vOptionBoutonThreshold, vOptionBoutonHeight, vOptionSomaThreshold,vOptionSomaThresholdValue
        ...
        vOptionProcessOnlyDendrites=var1.get()
    ...
    var1 = tk.IntVar(value=0)#process only dendrite segments
    ...
    tk.Checkbutton(window, text='Process ONLY Dendrite Segments',
    ...
    #For bouton detect and Spots display
            if max(vSegmentTypesWorking)==1 and vOptionFilamentToSpotsFill==1 or vOptionFilamentBoutonDetection==1 or vOptionFilamentCloseToSpots==1 or vOptionDendriteToDendriteContact==1 or vOptionFilamentToFilamentContact==1:
    ###########################
    ###########################
    #ReOrder segment
            #Reordering
    ...
    #Find distance along Filament for all boutons
        if vOptionFilamentBoutonDetection==1:
            ...
            if vBeginningVertex !=-1:
                ...
                #Create array of distance measures to original filament points
                wSpotToAllFilamentDistanceArrayOriginal=cdist(vAllNewSpotsBoutonsPositionXYZ,vFilamentsXYZ)
                # wSpotToAllFilamentDistanceArrayOriginal=wSpotToAllFilamentDistanceArrayOriginal- vFilamentsRadius -[max(vSpotsColocRadiusWorking[0])]*len(vFilamentsRadius)
                ...
                #loop for each spot within threshold
                #append new filament and create list of new spots
                for i in range (len(vAllNewSpotsBoutonsPositionXYZ)):
                    wNewFilamentsXYZ.append(vAllNewSpotsBoutonsPositionXYZ[i])
                    wNewFilamentsRadius.append(1)
                    wNewFilamentsEdges.append([wSpotsFilamentClosestDistancePointIndex[i],len(vFilamentsRadius)+i])
                    wNewFilamentsTypes.append(1)
            ...
            #Grab New Filament Spine Statistics for attachment point distance.
                ...
                xCompleteBoutonDistAlongFilamentStatWorking=[]
                #Collate all spots for each filament
                for i in range (len(vAllNewSpotsBoutonsRadius)):
                    #find nearest index
                    idx = (np.abs(np.array(vNewFilamentSpinePtPosX)-wNewSpotsOnFilament[i][0])).argmin()
                    xCompleteBoutonDistAlongFilamentStat.append(vNewFilamentSpineAttPtDist[vNewFilamentStatIdsDist.index(vNewFilamentStatIdsPosX[idx])])
                    xCompleteBoutonDistAlongFilamentStatWorking.append(vNewFilamentSpineAttPtDist[vNewFilamentStatIdsDist.index(vNewFilamentStatIdsPosX[idx])])
                #Add Spot object to scene for each filament object
        ...
        if vOptionFilamentCloseToSpots==1:
            ...
            if wSpotsAllIndexPerFilament!=[]:
                ...
                if vBeginningVertex !=-1:
                    ...
                    for i in range (len(wNewSpotsOnFilament)):
                        wNewFilamentsXYZ.append(wNewSpotsOnFilament[i])
            ...
            #Grab New Filament Spine Statistics for attachment point distance.
                    ...
                    for i in range (len(wSpotsAllIndexPerFilament)):
                        ...
                        # Test for the steps above
                        #i=0
                        # wNewSpotsOnFilament[i][0]
                        # vNewFilamentSpinePtPosX.index(wNewSpotsOnFilament[i][0])
                        # vNewFilamentStatIdsPosX[vNewFilamentSpinePtPosX.index(wNewSpotsOnFilament[i][0])]
                        # vNewFilamentStatIdsDist.index(vNewFilamentStatIdsPosX[vNewFilamentSpinePtPosX.index(wNewSpotsOnFilament[i][0])])
                        # vNewFilamentSpineAttPtDist[vNewFilamentStatIdsDist.index(vNewFilamentStatIdsPosX[vNewFilamentSpinePtPosX.index(wNewSpotsOnFilament[i][0])])]
            else:
                vNewSpotsvNewSpotsAlongFilament = vImarisApplication.GetFactory().CreateSpots()
    ...
    ############################################
            ...
            #Combine distance measure per filament (spine and dendrite)
                if qIsSpines==True and wCompleteShortestDistanceToSpine != []:
                ...
                if len(wCompleteSpotDistAlongFilamentStatWorking) == 0:
                    ...
                    vSpotsStatNames=[' Shortest Distance to Segment']*len(wSpotsAllIndexPerFilament)
                    ...
                    vSpotsStatNames=[' Shortest Distance to Segment']*len(vColocSpotToFilamentDistanceOnly)
                    vNewSpotsvNewSpotsAlongFilament.AddStatistics(vSpotsStatNames, vColocSpotToFilamentDistanceOnly,
                                                  vSpotsStatUnits, vSpotsStatFactors,
                                                  vSpotsStatFactorName, vSpotsvIds)
                    # if scount>1:
