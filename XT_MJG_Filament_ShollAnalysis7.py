    ...
    #<CustomTools>
        #<Menu>
            #<Submenu name="Filaments Functions">
                #<Item name="Filament Sholl Analysis7" icon="Python3">
                    #<Command>Python3XT::XT_MJG_Filament_ShollAnalysis7(%i)</Command>
                #</Item>
            #</Submenu>
       #</Menu>
       #<SurpassTab>
...
aImarisId=0
def XT_MJG_Filament_ShollAnalysis7(aImarisId):
    ...
    qInputBox.geometry("250x90")
    # qInputBoxTitle = Label(qInputBox, text = "Sholl Analysis")
    # qInputBoxTitle.pack()
    Label(qInputBox,text="Sholl Sphere Radius:").grid(row=0, column=0)
    Label(qInputBox,text="Number of Sholl Spheres:").grid(row=1, column=0)
    ##################################################################
    ...
    ##################################################################
    Entry1=Entry(qInputBox,justify='center',width=10)
    ...
    #loop for each sphere and create overall stat
        for i in range (len(zShollLabelsInt)-1):
            vOverallStatNames = [' '+ str(zShollLabelsInt[i+1]) + ' um Sholl Sum Dendrite Length']
