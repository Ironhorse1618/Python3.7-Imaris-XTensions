    ...
    #<CustomTools>
        #<Menu>
            #<Submenu name="Filaments Functions">
                #<Item name="Filament Sholl Analysis beta6" icon="Python3">
                    #<Command>Python3XT::XT_MJG_Filament_ShollAnalysis6_beta(%i)</Command>
                #</Item>
            #</Submenu>
       #</Menu>
       #<SurpassTab>
...
#New Sholl dendrites - a new filament object is created.  The original filament
#is spilt at every sholl intersection and anew stat identifying which sholl sphere
#it is in is created....FRom this you can total length in each sphere.
...
aImarisId=0
def XT_MJG_Filament_ShollAnalysis6_beta(aImarisId):
    ...
    #Create list of Sholl labels
            zShollLabels.append(str(aIntensityLowerThresholdManual))
    ...
    #Create new label/statistic to id
        vSpotPositionAllShollSpheresFlat = [x for xs in vSpotPositionAllShollSpheres for x in xs]
    ...
    #CreateLabels for each Sholl sphere
        ...
        zShollLabelsInt = [int(i) for i in zShollLabels]
        zShollLabelsInt.insert(0,0)#insert a zero at start of list
        vLabelIndices=[]
        for i in range (len(zShollLabelsInt)-1):
            # vLabelIndices = (np.where((zDistanceDendriteListValues[0] <= zShollLabelsInt[i+1]) &
            #                  (zDistanceDendriteListValues[0] > zShollLabelsInt[i])))[0].tolist()
            ##Creating a label for th dendrites ---NOT WORKING
            # wLabelList=[]
            # for j in range (len(vLabelIndices)):
            #     vLabelCreate = vFactory.CreateObjectLabel(zStatisticDendriteIds[vLabelIndices[j]], "Sholl Spheres", (str(zShollLabels[i])+ ' um Sholl'))
            #     wLabelList.append(vLabelCreate)
            # vNewFilaments.SetLabels(wLabelList)
    ...
    ###Creating new statistics for segmented dendrites.
            vNewFilamentsStatId.extend([zStatisticDendriteIds[i] for i in vLabelIndices])
            # vNewFilamentsStatShollSphere.extend([zShollLabelsInt[i+1]]*len(vLabelIndices))

    #Find the last ring of dendrites
        vLabelIndices = (np.where(zDistanceDendriteListValues[0] > zShollLabelsInt[i]))[0].tolist()
        vNewFilamentsStatId.extend([zStatisticDendriteIds[i] for i in vLabelIndices])
        # vNewFilamentsStatShollSphere.extend([zShollLabelsInt[i]+vShollRadius]*len(vLabelIndices))
    ...
    #######################
        ...
        vNewFilamentsStatNames=[' Filament ID']*(len(zStatisticDendriteIds))
    ...
    ########################
        ...
        vNewFilaments.AddStatistics(vNewFilamentsStatNames, vNewFilamentsStatShollSphere,
                              ...
                              vNewFilamentsStatFactorName, zStatisticDendriteIds)
    ########################
    #Calculate total length per ring
