...
# Features:
# 1.	Measure intensity of Filament objects
#     a.	Dendrite mean intensity (for each channel)
#     b.	Filament mean intensity (for each channel)
#     c.	Spine intensity (not just the spine head, the whole spine)
# 2.	Export Filament as a Spots object
#     a.	Place a spot at each point along the Filament, allowing for visualizing and measuring the localized intensity along segments
#         i.	It will fill gaps with spots between points that have spaces larger than the radius of dendrite at the point
# 3.	Detect Boutons (varicosities)
#     a.	Finds swellings along each dendrite segment and places a spot at that point
#     b.	Diameter calculation is required for this to work
#     c.	NOTE: Filament diameter must be rendered properly for this to work properly
# 4.	Find Spots Close to Filaments
#     a.	Detects the Spots within a defined distance threshold to each segment
#         i.	Quantifies number and density per segment
#         ii.	Quantifies number per Filament
#     b.	Calculates the Shortest Distance to Filament for each spot
#     c.	Measures colocalized Spot distances along dendrite to starting point
#         i.	Filament must have a Beginning Point
# 5.	Neuronal Self Avoidance
#     a.	Detects when dendrite segment contacts another dendrite segment from same Filament
#     b.	Quantifies number of contacts per dendrite segment
#         i.	Option to remove contacts near starting point
#     c.	Measures the length of each contact
#         i.	Represented by Spot size
# 6.	Neuron to Neuron contacts
#     a.	Detects when dendrite segment contacts a different segment from different Filament object
#     b.	Quantifies number of contacts per dendrite segment
#     c.	Measures the length each contact
#         i.	Represented by Spot size
#     d.	NOTE: current workflow is to have 2 separate Filament objects in the Surpass scene
...
def XT_MJG_Filament_Analysis26_beta(aImarisId):
    ...
    try:
        ...
        #Calculation of Mean Nearest Neighbor based on branch points and terminal points
            ...
            zRandomSpotsNode=[]
            if np.size(wFilamentBranchPoints, axis=0) >3:
                ...
                for i in range(3):
                    zRandomSpotsNode.append(np.random.uniform(low=zRandomLimitsMin[i], high=zRandomLimitsMax[i], size= len(wFilamentBranchPoints)))
                ...
                if zRandomSpotTestNode.any:
                    #find indices that are True
                    zBrokenRandomSpots=np.array(where(zRandomSpotTestNode==False))
                    for i in range (size(zBrokenRandomSpots)):
        ...
        ###############################################################################
            if np.shape(wFilamentTerminalPoints)[0] >3:
                zArrayTerminals=cdist(wFilamentTerminalPoints2,wFilamentTerminalPoints2)
                zArrayTerminalsMin=np.where(zArrayTerminals>0, zArrayTerminals, np.inf).min(axis=1)
                zAverageNNTerminalPoint=np.mean(zArrayTerminalsMin)
                for i in range(3):
                    zRandomSpotsTerminal.append(np.random.uniform(low=zRandomLimitsMin[i], high=zRandomLimitsMax[i], size= len(wFilamentTerminalPoints)))
                zRandomSpotsTerminal=np.transpose(np.array(zRandomSpotsTerminal))
                #Find if set of Random Spots falls inside Convex hull on Filame
                if (np.all(zArray[:,2] == zArray[0][2]))==True:
                    zArray=np.delete(zArray,2,1)
                ...
                #Keep True, replace false with new random spot
                if zRandomSpotTestTerminal.any:
                    #find indices that are True
                    zBrokenRandomSpots=where(zRandomSpotTestTerminal==False)
                    for i in range (size(zBrokenRandomSpots)):
                        #Create new random spot
                        zNEWRandomSpot=[]
                        for j in range(3):
                ...
                #Calculation of Mean Nearest Neighbor based on branch points and terminal points
                zArrayTerminals=cdist(zRandomSpotsTerminal,zRandomSpotsTerminal)
                zArrayTerminalsMin=np.where(zArrayTerminals>0, zArrayTerminals, np.inf).min(axis=1)
                zAverageNNTerminalPointRandom=np.mean(zArrayTerminalsMin)
                #Calculation of Regulatiry Index
                zAllFilamentsRegularityIndexTP.append(zAverageNNTerminalPoint/zAverageNNTerminalPointRandom)
            else:
