    ...
    #<CustomTools>
        #<Menu>
            #<Submenu name="Filaments Functions">
                #<Item name="Filament Sholl Analysis5_beta" icon="Python3">
                    #<Command>Python3XT::XT_MJG_Filament_ShollAnalysis5_beta(%i)</Command>
                #</Item>
            #</Submenu>
       #</Menu>
       #<SurpassTab>
...
aImarisId=0
def XT_MJG_Filament_ShollAnalysis5_beta(aImarisId):
# Create an ImarisLib object
    ...
    #Create a normalize color basd on the number of potential Sholl spheres
        # wApproxNumSpheres=round(max(wDistanceValuesMax)/vShollRadius).astype(int)
        wApproxNumSpheres=(max(wDistanceValuesMax)/vShollRadius).astype(int)
        wColorList=np.linspace(start=.1,stop=.9,num=wApproxNumSpheres)#.astype(int)
    ...
    #Create Sholl Spots for the current Sholl Sphere for current Filament
            ...
            if vShollSpotCount>0:
                ...
                vNewSpots.Set(vSpotPositionAllShollSpheresPerFilament[aShollIndex], [vFilamentsIndexT]*len(vSpotPositionAllShollSpheresPerFilament[aShollIndex]), [1]*len(vSpotPositionAllShollSpheresPerFilament[aShollIndex]))
                vNewSpots.SetName(str(aIntensityLowerThresholdManual) + 'um Sholl Sphere Filament (ID: ' + str(vFilamentIds[vFilamentCountActual-1])+')')
