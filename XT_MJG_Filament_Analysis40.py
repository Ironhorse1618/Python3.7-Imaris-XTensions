...
def XT_MJG_Filament_Analysis40(aImarisId):
    ...
    #ReOrder segment
        ...
        #flattten list
                ...
                zEndIndex=zIndiv[1]
                #zReOrderedFilamentPointIndexWorking.append(zStartIndex) #NOT USING THIS
            ...
            #start of loop for each dendrite segment
                for k in range (len(vSegmentRadiusWorking)-1):
                    ...
                    if k==0:
                        ...
                        #zReOrderedFilamentPointIndexWorking.append(zNextSegmentIndex)#NOT USING THIS
                ...
                vSegmentRadiusWorkingInserts=vSegmentRadiusWorking[:]
                vSegmentPositionsWorkingInserts=vSegmentPositionsWorking[:]
    
    
    
            # elapsed = time.time() - start
            # print('Reorder-'+str(elapsed)+ ' -'+str(vBranchIndex))
    ##############################################################################
    ##############################################################################
    #fill spots in filament point gaps
        ...
        #insert Radius at next spot
            ...
            #     vAllSegmentsPerFilamentPositionsWorkingInserts.extend(vSegmentPositionsWorkingInserts)
            #     vAllSegmentsTypesPerFilamentWorkingInserts.extend([max(vSegmentTypesWorking)]*len(vSegmentRadiusWorkingInserts))
            #     #vAllSegmentIdsPerFilamentInserts.extend([vSegmentIdWorking]*len(vSegmentRadiusWorkingInserts))#NOT IN USE
            #     #Create index for new segment points
            #     vAllSegmentIdsPerFilamentDendriteIndex.extend([vBranchIndex]*len(vSegmentRadiusWorkingInserts))
    
            
            # find spine attachment point and DELETE 1 or 2 points in VSegmentPositioWorkingInserts
            #This is so that shortest distance to spine is not including the attachment points which is large
            # if max(vSegmentTypesWorking)==1:
            # #Remove attachment point
            #     vSegmentPositionsWorkingInserts.pop(0)
            #     vSegmentRadiusWorkingInserts.pop(0)
            #Remove secon point from attachment point
                # vSegmentPositionsWorkingInserts.pop(0)
                # vSegmentRadiusWorkingInserts.pop(0)
    ...
    # Detect VaricositiesBoutons based on the radius of filled spot list
            if vOptionFilamentBoutonDetection==1 and max(vSegmentTypesWorking)==0:
                ...
                vNewSpotsBoutonsPositionXYZ = np.array(vSegmentPositionsWorkingInserts)[peaks.tolist()].tolist()                  
                vNewSpotsBoutonsRadius = np.array(vSegmentRadiusWorkingInserts)[peaks.tolist()].tolist()            
     ...
     # #Add Bouton Statistics to Filament object
        #Concatonate all boutons from all "real" filaments
                if vBranchIndex==vNumberOfSegmentsALL-1:#Do when last dendrite branch is processed for filament
    ...
    ###############################################################################
            ...
            if vOptionFilamentCloseToSpots==1:
                if max(vSegmentTypesWorking)==1:
                    ...
                    if np.any(vAllFilamentDendriteLengthIds == vSegmentIds[vBranchIndex]):
                        continue
                    if np.any(vAllFilamentSpineLengthIds == vSegmentIds[vBranchIndex]):
                        continue
    
                #Boolean test if all spot have the same time point
                if vSpotsColocTimeIndices.count(vSpotsColocTimeIndices[0])==len(vSpotsColocTimeIndices):
                    vSpotsColocPositionsWorking = vSpotsColocPositionsXYZ
                    vSpotsColocRadiusWorking = vSpotsColocRadius
                else:
                    # np.where(np.array(vSpotsColocTimeIndices) == vFilamentsIndexT)[0].tolist()
                    vSpotsColocPositionsWorking=np.array(vSpotsColocPositionsXYZ)[np.where(np.array(vSpotsColocTimeIndices) == vFilamentsIndexT)[0].tolist()].tolist()
                    vSpotsColocRadiusWorking=np.array(vSpotsColocRadius)[np.where(np.array(vSpotsColocTimeIndices) == vFilamentsIndexT)[0].tolist()].tolist()
        ...
        #Isolate spots on dendrites and spines
                if max(vSegmentTypesWorking)==0:
                    ...
                    wSpotsIndexPerSegment=np.where(wShortestDistanceToSegment<=vOptionFilamentSpotThreshold)[0].tolist()
                ...
                wCompleteNumberSpotsonDendritePerFilament.append(np.size(np.unique(np.array(wSpotsAllIndexPerFilament)))
    )
                
                
                
    #Progress bar for dendrite segments
            progress_bar['value'] = int((vBranchIndex+1)/vNumberOfSegmentsALL*100) #  % out of 100
            master.update()
    ###############################################################################
    ###############################################################################
    ###############################################################################
    ...
    ##############
        ...
        #Find and search for indices of All points that match current node/branch point or startpoint
            ...
            if xStartingBranchSharingNodes:
    ...
    ##############################################################################
                ...
                progress_bar3['value'] = int((vBranchIndex+1)/vNumberOfSegmentsALL*100) #  % out of 100
                master.update()
    #############################################################################
    ...
    # #Make spot position conect to filament as spine attachment point
            ...
            if wSpotsAllIndexPerFilament!=[]:
                ...
                if vBeginningVertex !=-1:
                    ...
                    wNewFilamentsXYZ=list(vFilamentsXYZ)
                    #wNewFilamentsTypes=list(vTypes)
                    wNewFilamentsTypes =  [0]*(len(vTypes))
    ...
    if vOptionBranchPoints == 1 and vOptionMergePoints == 1:
        ...
        vNewSpotsBranchPointsPerFilamentComplete.Set(wFilamentBranchPointsNEW,
                              [vFilamentsIndexT]*len(wFilamentBranchPointsNEW),
                              [1]*len(wFilamentBranchPointsNEW))
    ...
    ###########
        ...
        vLabelIndices=list(range(len(wFilamentBranchPointsNEW)))
        wLabelList=[]
    ...
    if vOptionTerminalPoints == 1 and vOptionMergePoints == 1:
        ...
        vNewSpotsTerminalPointsPerFilamentComplete.Set(wFilamentTerminalPointsNEW,
                              [vFilamentsIndexT]*len(wFilamentTerminalPointsNEW),
                              [1]*len(wFilamentTerminalPointsNEW))
