...
#             <Item name="2D Shape Analysis6 beta" icon="Python3">
#                 <Command>Python3XT::XT_MJG_Shape_Analysis6_beta(%i)</Command>
#             </Item>
#         </Submenu>
#     </Menu>
#     <SurpassTab>
...
#aImarisId=0
def XT_MJG_Shape_Analysis6_beta(aImarisId):
    ...
    tk.Checkbutton(window, text='Create midline Surface (Longer Processing Time)',
                    variable=var1, onvalue=1, offvalue=0).grid(row=1, column=0, padx=40,sticky=W)
    tk.Checkbutton(window, text='Create Disc',
    ...
    #Set Donut FinalMask (donut) to new channel
        if vOptionMakeRings==1:
            if vOptionMakeDisc==1:
                ...
                if vSurfaceIndexMiddle!=0:
                    vImage.SetDataSliceFloats(vSliceNumpy.tolist(),vSurfaceIndexMiddle-1,vSizeC,vAllTimeIndices[vSurfaceIndex])
                else:
                    vImage.SetDataSliceFloats(vSliceNumpy.tolist(),vSurfaceIndexMiddle,vSizeC,vAllTimeIndices[vSurfaceIndex])
            else:
