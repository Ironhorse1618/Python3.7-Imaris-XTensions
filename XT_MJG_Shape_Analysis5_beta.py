...
def XT_MJG_Shape_Analysis5_beta(aImarisId):
    ...
    #Find slice closest to the center of mass
        ...
        if vSurfaceIndexMiddle!=0:
            vSlice = zMaskSingleSurface.GetDataSliceFloats(vSurfaceIndexMiddle-1, 0, 0)
        else:
            vSlice = zMaskSingleSurface.GetDataSliceFloats(vSurfaceIndexMiddle, 0, 0)
    ...
    ####################################################
    if vOptionMakeRings==1:
        vSurfaceStatNames=[' Perimeter midline (convexhull)']*(len(vNewStatPerimeter))
    ...
    #                                   vSurfaceStatFactorName, vSurfaceStatvIds)
    ####################################################
        vSurfaceStatNames=[' Perimeter midline - Contour vertices']*(len(vNewStatPerimeter))
