...
#             <Item name="2D Shape Analysis10" icon="Python3">
#                 <Command>Python3XT::XT_MJG_Shape_Analysis10(%i)</Command>
#             </Item>
#         </Submenu>
#     </Menu>
#     <SurpassTab>
...
def XT_MJG_Shape_Analysis10(aImarisId):
    ...
    Checkbutton(qInputBox, text='Create Perimeter Rings at midline (Longer Processing Time)',
                    variable=var1, onvalue=1, offvalue=0).grid(row=1, column=0, padx=40,sticky=W)
    Checkbutton(qInputBox, text='Fill Surfaces',
    ...
    tk.Label(qInputBox, font="bold", text='-------------------------').grid(row=6,column=0,sticky=W)
    
    tk.Label(qInputBox, font="bold", text='Statistics Calculated:\n').grid(row=7,column=0,sticky=W)
    tk.Label(qInputBox, text='Perimeter midline\n'
                             'Area cross section (midline)\n'
                             'Compactness\n'
                             'Circularity\n'
                             'Convexity\n'
                             ...
                             'Feret Diameter\n'
                             'Chord Max Length\n'
                             'Intensity border ring').grid(row=8,column=0,sticky=W)
    ...
    vNewStatConvexity=[]
    vNewStatSolidity=[]
    vNewStatFeretDiameterMax = []
    vNewStatFeretDiameterMax90 = []
    ########################
    ########################
    ...
    for vSurfaceIndex in range (vNumberOfSurfaces):
        vSurpassObjectIndexT = vSurfaces.GetTimeIndex(vSurfaceIndex)
    ...
    #Find slice closest to the center of mass
        ...
        #Test if the surface is valid and has valid mask
        if zMaskTest==0:
            qIsBadSurface=True
        ...
        ## find the non-zero min-max coords of canny
            ...
            ##Erode border to clean up edges
            vSlicecropped=ndimage.grey_erosion(vSlicecropped, size=(2,1))
    
            #Run Variance filter thru scipy to find edges of the binary
            if vOptionExpandBorderIntensity==1:
                vVarianceFilterResult=generic_filter(vSlicecropped, np.std, size=4)
            else:
                vVarianceFilterResult=generic_filter(vSlicecropped, np.std, size=2)
    
    
            # plt.imshow(vSlicecroppedfilled)
            # plt.imshow(vSlicecropped)
            # plt.imshow(vSlice)
            # plt.imshow(vSliceNumpyNew)
            # plt.imshow(vVarianceFilterResult)
    ...
    #Set Donut FinalMask (donut) to new channel
        if vOptionMakeRings==1:
            if vOptionMakeDisc==1:
                ...
                if vSurfaceIndexMiddle!=0:
                    vDataSet.SetDataSliceFloats(vSliceNumpy.tolist(),vSurfaceIndexMiddle-1,0,vAllTimeIndices[vSurfaceIndex])
                else:
                    vDataSet.SetDataSliceFloats(vSliceNumpy.tolist(),vSurfaceIndexMiddle,0,vAllTimeIndices[vSurfaceIndex])
            else:
                ...
                if vSurfaceIndexMiddle!=0:
                    vDataSet.SetDataSliceFloats(vFinalMaskToList,vSurfaceIndexMiddle-1,0,vAllTimeIndices[vSurfaceIndex])
                else:
                    vDataSet.SetDataSliceFloats(vFinalMaskToList,vSurfaceIndexMiddle,0,vAllTimeIndices[vSurfaceIndex])
    ...
    #Set Donut result to new channel - If the option is checked
        ...
        #Make single Surface from Donut channel using ROI. no smoothing
            vLowerThreshold=10
            vPerimeterRingsWorking = ip.DetectSurfacesWithUpperThreshold(vDataSet,
                                                                    [[0,
                                                                      0,
    ...
    #Test if image is 2D
    vStatBBOOLongestPrincipalSet = vSurfaces.GetStatisticsByName('BoundingBoxOO Length C')
    qTestif3D = vStatBBOOLongestPrincipalSet.mValues
    
    if vStatBBOOLongestPrincipalSet.mValues == []:
    #If image is opened in 2D
        vStatBBOOLongestPrincipalSet = vSurfaces.GetStatisticsByName('BoundingBoxOO Length B')
        vStatBBOOSecondLongestAxisSet = vSurfaces.GetStatisticsByName('BoundingBoxOO Length A')
    else:
    #If image is opened in 3D
        vStatBBOOLongestPrincipalSet = vSurfaces.GetStatisticsByName('BoundingBoxOO Length C')
        vStatBBOOSecondLongestAxisSet = vSurfaces.GetStatisticsByName('BoundingBoxOO Length B')
    
    #####
    vNewStatFeretDiameterMax = vStatBBOOLongestPrincipalSet.mValues
    vNewStatFeretDiameterMax90 = vStatBBOOSecondLongestAxisSet.mValues
    vNewStatFeretDiameterMaxIds = vStatBBOOLongestPrincipalSet.mIds
    vNewStatFeretDiameterMax90Ids = vStatBBOOSecondLongestAxisSet.mIds
